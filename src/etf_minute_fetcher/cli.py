...
def _resolve_symbols(args: argparse.Namespace) -> list[str]:
    ...
    invalid_filters = [
        flag for flag, value in universe_only_filters.items() if value and not universe_name
    ]
    if invalid_filters:
    ...
    if symbols_arg:
        providers.append(
            ExplicitUniverse(
                [symbol.strip() for symbol in symbols_arg.split(",") if symbol.strip()]
            )
        )
    if symbols_file:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="etf-min", description="抓取 ETF 分钟行情")
    ...
    parser.add_argument("--universe", choices=("cn-etf",), help="自动发现沪深 ETF")
    parser.add_argument(
        "--exchange",
        choices=("SH", "SZ"),
        help="只筛选自动发现集合中的指定交易所 ETF",
    )
    parser.add_argument("--name-contains", help="按 ETF 名称包含关系筛选自动发现集合")
    parser.add_argument("--fund-type", help="按基金类型精确筛选，如 股票型、债券型")
    parser.add_argument("--as-of", help="按 YYYYMMDD 使用历史 ETF 快照")
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="未指定 --start 时，从 --end 往前覆盖的自然日数",
    )
    parser.add_argument(
        "--period",
    ...
    parser.add_argument(
        "--out",
        required=True,
        help="输出根目录（将创建 ts_code/trade_date=* 分区）",
    )
    parser.add_argument("--no-skip", action="store_true", help="覆盖已存在的交易日分区")
    parser.add_argument("--workers", type=int, default=4, help="最大并发 ETF 任务数，默认 4")
    parser.add_argument(
        "--rate-limit",
        type=float,
        default=2.0,
        help="每秒最多启动的 ETF 任务数，0 表示不限速",
    )
    parser.add_argument(
        "--symbol-attempts",
        type=int,
        default=2,
        help="失败 ETF 的批量级最大尝试次数",
    )
    parser.add_argument(
        "--retry-delay",
        type=float,
        default=2.0,
        help="失败重试轮次之间的基础等待秒数",
    )
    parser.add_argument(
        "--checkpoint",
        help="断点 JSON，默认 <out>/.download-checkpoint.json",
    )
    parser.add_argument(
        "--stats-file",
        help="下载汇总 JSON，默认 <out>/.download-summary.json",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
        help="忽略已有断点，重新调度所有 ETF",
    )
    return parser
...
def _print_plan(
    ...
    print(
        f"[etf-min] 区间 {trade_dates[0]}~{trade_dates[-1]}"
        f"（{len(trade_dates)} 个自然日），period={args.period}"
    )
    print(
    ...
    if args.period == "1":
        print("[etf-min] 注意：1 分钟公开接口通常只提供最近 5 个交易日，更早日期会显示 empty")
...
def _run_download(
    ...
    return DownloadEngine(config).run(
        symbols,
        trade_dates,
        period=args.period,
...
def _report_summary(summary: DownloadSummary) -> int:
    print(
    ...
    if summary.written_partitions == 0 and summary.skipped_partitions == 0:
        print(
            "ERROR: 本次没有产生任何数据分区，请检查日期窗口、ETF 代码或上游接口状态",
            file=sys.stderr,
        )
        return 3
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not symbols:
        print(
            "ERROR: 必须通过 --symbols、--symbols-file 或 --universe 指定至少一只 ETF",
            file=sys.stderr,
        )
        return 2
    ...
    _print_plan(args, symbols, trade_dates, config, output_dir)

    try:
        summary = _run_download(args, symbols, trade_dates, config, output_dir)
    except (ValueError, OSError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 2
    return _report_summary(summary)
