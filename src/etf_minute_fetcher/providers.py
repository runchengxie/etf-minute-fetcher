"""Minute-bar provider abstractions and default AKShare/EastMoney/Sina stack."""
...
class MinuteDataProvider(Protocol):
    """Return normalized minute bars for one ETF and date range."""
...
def normalize_frame(raw: pd.DataFrame | None, ts_code: str) -> pd.DataFrame:
    """Normalize an upstream frame to the stable project schema."""
...
def filter_frame_to_range(frame: pd.DataFrame, start_trade_date: str, end_trade_date: str) -> pd.DataFrame:
    if frame.empty:
...
class FallbackMinuteProvider:
    """Default provider chain: AKShare -> EastMoney curl -> Sina curl."""
